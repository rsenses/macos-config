...
6. Only after explicit approval, invoke exactly the approved planner profile (or the exact approved model/thinking configuration), nothing else. Give the planner the canonical contract and relevant evidence, not the raw conversation. The planner returns one executable checklist, not duplicate file/task lists; its output is provisional until incorporated. The principal remains coordinator; the planner has no nested subagent tools.
