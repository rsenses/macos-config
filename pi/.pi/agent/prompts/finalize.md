...
2. **Changelog**: If `CHANGELOG.md` exists at the project root, verify it has an up-to-date SemVer-aligned Keep a Changelog entry for this job's user-visible changes. Add or update it before reporting `ready to ship`; if it is missing or stale, report `not ready to ship`.
...
5. **Reporting**: Prefer deltas and unresolved items over reprinting the whole plan or any inventory already captured in it.
6. **Outcome**: Propose a `type(scope): summary` commit message based on the task goal. Do not commit automatically.
