...
This skill is for the top-level orchestrator (the assistant speaking to the user), not for subagents.
...
- **`subagent` delegate** — lightweight passthrough when you want the parent model with minimal extra context.
- **`subagent` oracle** — high-context decision check to catch drift before committing to a path.
- **`subagent` worker** — isolated code changes. Tools: `read`, `write`, `edit`, `bash`. Use when the change is well-specified and doesn't need back-and-forth.
- **`subagent` context-builder** — synthesizes requirements and codebase context into a handoff-ready prompt.

- If the target repo exposes MCP servers, prefer them for research when relevant; this is especially important for `researcher`, but other subagents may use them too.
