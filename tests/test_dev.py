...
No tocan /tmp/dev real, Caddy, sudo, PHP, npm ni proyectos del usuario: todas
las rutas se redirigen a un directorio temporal mediante las variables DEV_* del
propio script, y los comandos externos (curl, caddy, sudo, lsof, ps, php, npm)
se sustituyen por dobles falsos. Los únicos procesos reales que se señalizan
son `sleep` creados por la propia prueba.
...
if printf '%s' "$*" | grep -q -- ' -d cwd '; then
  ...
  if [ -n "$pid" ] && [ -f "$FAKE_PS_DIR/$pid.cwd" ]; then
    printf 'n%s\n' "$(cat "$FAKE_PS_DIR/$pid.cwd")"
  elif [ -n "${{DEV_FAKE_CWD:-}}" ]; then
    printf 'n%s\n' "$DEV_FAKE_CWD"
  fi
  exit 0
fi
...
if [ "$what" = "pid=,ppid=" ]; then
  for ppid_file in "$dir"/*.ppid; do
...
exec sleep 300
''')
        # npm falso: mantiene un proceso hijo Node sintético para verificar el
        # árbol npm -> Node que escucha el puerto.
        self._write_fake("npm", f'''#!/bin/sh
 dir="$FAKE_PS_DIR"
 port=""
 while [ "$#" -gt 0 ]; do
   case "$1" in
 ...
 mkdir -p "$dir"
 printf 'npm run dev -- --host 127.0.0.1 --port %s\n' "$port" >> "{self.calls}"
 printf 'npm run dev -- --host 127.0.0.1 --port %s\n' "$port" > "$dir/$$.cmd"
 printf 'FAKE-START\n' > "$dir/$$.lstart"
 (
   sleep 300
 ...
 child="$!"
 printf 'node vite --host 127.0.0.1 --port %s\n' "$port" > "$dir/$child.cmd"
 printf 'FAKE-START\n' > "$dir/$child.lstart"
 printf '%s\n' "$$" > "$dir/$child.ppid"
 printf '%s\n' "$$" >> "{self.tmp}/fakenode.pids"
 printf '%s\n' "$child" >> "{self.tmp}/fakenode.pids"
 cleanup() {{
...
''')
        ...
        self.addCleanup(self._kill_fake_processes)
    ...
    def _kill_fake_processes(self):
        for marker in (self.tmp / "fakephp.pids", self.tmp / "fakenode.pids"):
    ...
    def run_dev(self, *args, cwd=None, env_extra=None):
        ...
        target = Path(cwd if cwd is not None else self.project).resolve()
        env["DEV_FAKE_CWD"] = str(target)
        if env_extra:
    ...
    def write_state(self, project_dir, pid, port, pid_start="", server_type="", work_dir="", pgid=""):
        ...
        if server_type:
            values.append(f"SERVER_TYPE={server_type}")
        if work_dir:
            values.append(f"WORK_DIR={work_dir}")
        if pgid:
            values.append(f"PGID={pgid}")
        path.write_text("\n".join(values) + "\n")
        return path
    ...
    def set_ps_identity(self, pid, command, start=None, cwd=None, parent=None):
        ...
        if cwd is not None:
            (self.psdata / f"{pid}.cwd").write_text(str(cwd) + "\n")
        if parent is not None:
            (self.psdata / f"{pid}.ppid").write_text(str(parent) + "\n")
    ...
    def fake_node_pids(self):
        marker = self.tmp / "fakenode.pids"
        if not marker.exists():
            return []
        return [int(line) for line in marker.read_text().split() if line]
    ...
    def test_down_all_stops_all_projects_with_single_reload(self):
        ...
        state1 = self.write_state(
            self.project_real, pid=self.dead_pid(), port=8123, server_type="php"
        )
        state2 = self.write_state(
            project2_real, pid=self.dead_pid(), port=8124, server_type="node"
        )
    ...
    def test_up_detects_node_assigns_port_and_forwards_arguments(self):
        self.mark_caddy_running()
        self.mark_caddy_ok()
        (self.project / "package.json").write_text('{"scripts":{"dev":"vite"}}\n')

        result = self.run_dev("up", env_extra={"DEV_FAKE_LSOF_LISTEN_FROM": "3"})

        self.assertEqual(result.returncode, 0, result.stderr)
        self.assertIn("https://proyecto.test", result.stdout)
        self.assertIn("127.0.0.1:8000", result.stdout)
        self.assertIn("npm run dev -- --host 127.0.0.1 --port 8000", self.calls.read_text())
        state = self.state_file_for(self.project_real)
        values = dict(line.split("=", 1) for line in state.read_text().splitlines() if "=" in line)
        self.assertEqual(values["SERVER_TYPE"], "node")
        self.assertEqual(values["PORT"], "8000")
        self.assertIn("reverse_proxy 127.0.0.1:8000", (self.sites / "proyecto.caddy").read_text())

    def test_status_reports_active_node_server(self):
        self.mark_caddy_running()
        self.mark_caddy_ok()
        (self.project / "package.json").write_text('{"scripts":{"dev":"nuxt dev"}}\n')
        up = self.run_dev("up", env_extra={"DEV_FAKE_LSOF_LISTEN_FROM": "3"})
        self.assertEqual(up.returncode, 0, up.stderr)

        result = self.run_dev("status")

        self.assertEqual(result.returncode, 0, result.stderr)
        self.assertIn("Servidor Node: activo en 127.0.0.1:8000", result.stdout)

    def test_down_stops_node_and_its_listener_child(self):
        self.mark_caddy_running()
        self.mark_caddy_ok()
        (self.project / "package.json").write_text('{"scripts":{"dev":"vite"}}\n')
        up = self.run_dev("up", env_extra={"DEV_FAKE_LSOF_LISTEN_FROM": "3"})
        self.assertEqual(up.returncode, 0, up.stderr)
        node_pids = self.fake_node_pids()
        ...
        for pid in node_pids:
            self.assertTrue(self.wait_pid_gone(pid), f"el proceso Node {pid} debe quedar detenido")

    def test_down_does_not_kill_node_process_from_another_directory(self):
        proc = self.spawn_sleep()
        other_dir = self.tmp / "otro-proyecto"
        other_dir.mkdir()
        self.set_ps_identity(
            proc.pid,
        ...
        self.assertEqual(result.returncode, 1, result.stderr)
        self.assertIn("no corresponde a un servidor Node", result.stderr)
        self.assertIsNone(proc.poll(), "no debe señalizar un npm de otro proyecto")
        self.assertFalse(self.state_file_for(self.project_real).exists())
